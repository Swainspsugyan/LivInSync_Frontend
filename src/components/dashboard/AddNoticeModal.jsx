...
export default function AddNoticeModal({ open, onClose, onCreated }) {
  const { t } = useI18n()
  const [values, setValues] = useState({ title: '', description: '', createdAt: todayISO() })
  ...
  const submit = (event) => {
    ...
    const created = addNotice(values)
    setValues({ title: '', description: '', createdAt: todayISO() })
    setError('')
    onCreated?.(created)
    onClose()
  }
  ...
  return (
    <div className="fixed inset-0 z-40 flex items-end justify-center bg-slate-900/30 p-4 sm:items-center">
      <form onSubmit={submit} className="dash-panel-card w-full max-w-lg p-4 sm:p-5" role="dialog" aria-modal="true" aria-labelledby="notice-form-title">
        <h3 id="notice-form-title" className="text-sm font-semibold text-slate-900 dark:text-white">
          {t('home.addNotice')}
        </h3>
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          <DashBtn type="submit">{t('home.publishNotice')}</DashBtn>
